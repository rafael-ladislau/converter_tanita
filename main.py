...
sorted_headers = ['Date','"Weight (kg)"','BMI','"Body Fat (%)"','"Visc Fat"','"Muscle Mass (kg)"','"Muscle Quality"','"Bone Mass (kg)"','"BMR (kcal)"','"Metab Age"','"Body Water (%)"','"Physique Rating"','"Muscle mass - right arm"','"Muscle mass - left arm"','"Muscle mass - right leg"','"Muscle mass - left leg"','"Muscle mass - trunk"','"Muscle quality - right arm"','"Muscle quality - left arm"','"Muscle quality - right leg"','"Muscle quality - left leg"','"Muscle quality - trunk"','"Body fat (%) - right arm"','"Body fat (%) - left arm"','"Body fat (%) - right leg"','"Body fat (%) - left leg"','"Body fat (%) - trunk"','"Heart rate"']
def submit_text():
    ...
    if not matches:
        ...
        for header in sorted_headers:
            final_result[header] = results[header]

        filename = filedialog.asksaveasfilename(defaultextension=".csv", initialfile="resultado.csv")  # get filename from user
        if filename:
            with open(filename, 'w', newline='') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=final_result.keys(), quoting=csv.QUOTE_NONE, quotechar="'")
                writer.writeheader()
